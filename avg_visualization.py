...
def slide_window_success_rate_no_overlap(n_episode, window, is_success, path):
    success_rate = np.array([np.mean(is_success[step * window: (step + 1) * window])
                             for step in range(n_episode // window)])
...
def visualize(path, args=None):  # path = new_test
    ...
    for dir_ in all_dir:  # 30 folders in each mechanism folder
        json_files = sorted(glob.glob(os.path.join(
            'logs', path, dir_, '*')))  # 500개의  json file
    ...
    slide_window_success_rate_no_overlap(
        len(summarized_success), args.window, np.array(summarized_success), path)
